...
export function ColourLayer(props: ColourLayerProps) {
    return (
        <div style={{ width: props.width, height: 500, position: 'absolute', display: 'inline-block', zIndex: props.height, objectFit: 'scale-down', background: "red", WebkitMaskImage: "url(" + props.src + ")", maskImage: "url(" + props.src + ")", WebkitMaskRepeat: "no-repeat", maskRepeat: "no-repeat", WebkitMaskSize: "contain", maskSize: "contain" }} />
    );
}
...
export default function Repogotchi(props: RepogotchiProps) {
    ...
    const earsClrMask = require('../sprite/colormask.png');

    const earsSrc = require('../sprite/ears' + props.repo.Ears + '.png');
    const earsClrSrc = require('../sprite/earcolour' + props.repo.Ears + '.png');
    const bodySrc = require('../sprite/body' + props.repo.Body + '.png');
    const bodyClrSrc = require('../sprite/bodycolour' + props.repo.Body + '.png');
    const mouthSrc = require('../sprite/mouths/mouth' + props.repo.Mouth + '.png');
    const eyesSrc = require('../sprite/eyes' + props.repo.Eyes + '.png');
        ...
        <ThemeProvider theme={theme}>
            <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center" style={{ marginTop: 50 }}>
                ...
                <Box display="inline" alignItems="center" justifyContent="center" position="relative" style={{ width: "100%", height: 500 }}>
                    <ColourLayer height={1} src={earsClrMask} width={imgWidth} />
